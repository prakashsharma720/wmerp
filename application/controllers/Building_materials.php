...
// Show login page	
	...
	public function index($id = NULL) 
	{
		...
		$data = array();
		 ...
		 if (isset($result['code']) && $result['code']) {
            ...
            if($voucher_no<10){
            $pm_id_code='BM000'.$voucher_no;
            }
            else if(($voucher_no>=10) && ($voucher_no<=99)){
              $pm_id_code='BM00'.$voucher_no;
            }
            else if(($voucher_no>=100) && ($voucher_no<=999)){
              $pm_id_code='BM0'.$voucher_no;
            }
            else{
              $pm_id_code='BM'.$voucher_no;
            }
           //print_r($pm_id_code);exit;
			...
			$data['title'] = 'Building Material Master';
			$data['building_materials'] = $this->categories_model->packingmaterialsList( $data['categories_id']);
	...
	public function add_newPM() {
		...
		else 
		{
			...
			if ($result == TRUE) {
			$this->session->set_flashdata('success', 'Building Materials Added Successfully !');
			redirect('/Building_materials/index', 'refresh');
			//$this->fetchSuppliers();
			} else {
			$this->session->set_flashdata('failed', 'Already Exists ,Building Materials Not Inserted !');
			redirect('/Building_materials/index', 'refresh');
			}
		} 
	}
	...
	public function editPM($id) {
		...
		else  
		{
			...
			if ($result == TRUE) {
				...
				if ($result1 == TRUE) {
						$this->session->set_flashdata('success', 'Building Materials Updated Successfully !');
						redirect('/Building_materials/index', 'refresh');
					//$this->fetchSuppliers();
					} else {
						$this->session->set_flashdata('failed', 'No Changes in Building Materials!');
						redirect('/Building_materials/index', 'refresh');
					}
