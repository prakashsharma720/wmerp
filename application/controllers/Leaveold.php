...
// Show login page	
	...
	public function index() 
	{
		...
		if($this->input->get()) {
			...
			if(($data['designation_id']==1)|| ($data['designation_id']==2 )|| ($data['designation_id']==4)){
			...
			if(($data['designation_id']==1)|| ($data['designation_id']==2 )|| ($data['designation_id']==4)){
			  $data['leaves']=$this->Leave_model->LeaveListCSV(); 
			}
			else{
			// echo $login_id;
			$conditions['login_id'] = $login_id;
			...
			// $data['daily_tasks'] 		= $this->task_model->TaskListCSV();
		   $data['filtered_value'] = "";
		 }
	...
	public function add_new_item() {
		...
		else 
		{
			...
			$data1 = array(
				...
				'message' => 'This is massage to get leave.',
			...
			$this->Leave_model->add_notification($data1);
	...
	public function leave_action($id) {
		...
		$result = $this->Leave_model->leave_update($data1, $id);
		...
		if($result == TRUE) {
			...
			if($this->input->post('leave_status') == 'Approved'){
				...
				$this->Leave_model->leave_taken($data);
			}
