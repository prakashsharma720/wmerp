...
// Show login page	
	...
	public function index($id = NULL) 
	{
		...
		$data = array();
	       ...
	       if (isset($result['code']) && $result['code']) {
            ...
            if($voucher_no<10){
            $pm_id_code='ST000'.$voucher_no;
            }
            else if(($voucher_no>=10) && ($voucher_no<=99)){
              $pm_id_code='ST00'.$voucher_no;
            }
            else if(($voucher_no>=100) && ($voucher_no<=999)){
              $pm_id_code='ST0'.$voucher_no;
            }
            else{
              $pm_id_code='ST'.$voucher_no;
            }
           //print_r($pm_id_code);exit;
			}
			$data['stationary_code_view']=$pm_id_code;
			...
			$data['title'] = 'Stationery Master';
			$data['stationeries'] = $this->categories_model->packingmaterialsList( $data['categories_id']);
	...
	public function add_newPM() {
		...
		else 
		{
			...
			if ($result == TRUE) {
			$this->session->set_flashdata('success', 'Stationery Added Successfully !');
			redirect('/Stationery/index', 'refresh');
			//$this->fetchSuppliers();
			} else {
			$this->session->set_flashdata('failed', 'Already Exists ,Stationery Not Inserted !');
			redirect('/Stationery/index', 'refresh');
			}
		} 
	}
	...
	public function editPM($id) {
		$this->form_validation->set_rules('name', 'Stationery Name', 'required');
		//$this->form_validation->set_rules('bag_packing', 'Packing', 'required');
		if ($this->form_validation->run() == FALSE) 
		{
			if(isset($this->session->userdata['logged_in'])){
		...
		else  
		{
			...
			if ($result == TRUE) {
				...
				if ($result1 == TRUE) {
					$this->session->set_flashdata('success', 'Stationery Updated Successfully !');
					redirect('/Stationery/index', 'refresh');
					//$this->fetchSuppliers();
					} else {
					$this->session->set_flashdata('failed', 'No Changes in Stationery!');
					redirect('/Stationery/index', 'refresh');
					}
