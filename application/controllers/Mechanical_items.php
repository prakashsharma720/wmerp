...
// Show login page	
	...
	public function index($id = NULL) 
	{
		...
		$data = array();
            ...
            if($voucher_no<10){
            $pm_id_code='MI000'.$voucher_no;
            }
            else if(($voucher_no>=10) && ($voucher_no<=99)){
              $pm_id_code='MI00'.$voucher_no;
            }
            else if(($voucher_no>=100) && ($voucher_no<=999)){
              $pm_id_code='MI0'.$voucher_no;
            }
            else{
              $pm_id_code='MI'.$voucher_no;
            }
           //print_r($pm_id_code);exit;
			}
			$data['mi_code_view']=$pm_id_code;
		/* 	if (isset($result['grade_id']) && $result['grade_id']) :
			...
			$data['title'] = 'Mechanical Items Master';
			$data['mechanical_items'] = $this->categories_model->packingmaterialsList( $data['categories_id']);
	...
	public function add_newPM() {
		...
		else 
		{
			...
			if ($result == TRUE) {
			$this->session->set_flashdata('success', 'Mechanical Items Added Successfully !');
			redirect('/Mechanical_items/index', 'refresh');
			//$this->fetchSuppliers();
			} else {
			$this->session->set_flashdata('failed', 'Already Exists ,Mechanical Items Not Inserted !');
			redirect('/Mechanical_items/index', 'refresh');
			}
		} 
	}
	...
	public function editPM($id) {
		$this->form_validation->set_rules('name', 'Mechanical Items Name', 'required');
		//$this->form_validation->set_rules('bag_packing', 'Packing', 'required');
		if ($this->form_validation->run() == FALSE) 
		{
			if(isset($this->session->userdata['logged_in'])){
		...
		else  
		{
			...
			if ($result == TRUE) {
				...
				if ($result1 == TRUE) {
						$this->session->set_flashdata('success', 'Mechanical Items Updated Successfully !');
						redirect('/Mechanical_items/index', 'refresh');
					//$this->fetchSuppliers();
					} else {
						$this->session->set_flashdata('failed', 'No Changes in Mechanical Items!');
						redirect('/Mechanical_items/index', 'refresh');
					}
