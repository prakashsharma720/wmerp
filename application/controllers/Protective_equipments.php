...
Class Protective_equipments extends MY_Controller {
...
// Show login page	
	...
	public function index($id = NULL) 
	{
		...
		$data = array();
            ...
            if($voucher_no<10){
            $pm_id_code='PEQ000'.$voucher_no;
            }
            else if(($voucher_no>=10) && ($voucher_no<=99)){
              $pm_id_code='PEQ00'.$voucher_no;
            }
            else if(($voucher_no>=100) && ($voucher_no<=999)){
              $pm_id_code='PEQ0'.$voucher_no;
            }
            else{
              $pm_id_code='PEQ'.$voucher_no;
            }
           //print_r($pm_id_code);exit;
			...
			$data['title'] = 'Protective Equipments Master';
			$data['protective_equipments'] = $this->categories_model->packingmaterialsList( $data['categories_id']);
			$data['categories'] = $this->categories_model->getMasterCategories();
			$data['units'] = $this->categories_model->getUnits();
			//echo var_dump($data['students']);
			//print_r($data['name']);exit;
			$this->template->load('layout/template','protective_equipment_view',$data);
		/*}
		else{
	...
	public function add_newPM() {
		...
		$this->form_validation->set_rules('name', 'Protective Equipments Name', 'required');
		...
		else 
		{
			...
			if ($result == TRUE) {
			$this->session->set_flashdata('success', 'Protective Equipments Added Successfully !');
			redirect('/Protective_equipments/index', 'refresh');
			//$this->fetchSuppliers();
			} else {
			$this->session->set_flashdata('failed', 'Already Exists ,Protective Equipments Not Inserted !');
			redirect('/Protective_equipments/index', 'refresh');
			}
		} 
	}
	...
	public function editPM($id) {
		...
		else  
		{
			...
			if ($result == TRUE) {
				...
				if ($result1 == TRUE) {
					$this->session->set_flashdata('success', 'Protective Equipments Updated Successfully !');
					redirect('/Protective_equipments/index', 'refresh');
					//$this->fetchSuppliers();
					} else {
						$this->session->set_flashdata('failed', 'No Changes in Protective Equipments!');
						redirect('/Protective_equipments/index', 'refresh');
					}
