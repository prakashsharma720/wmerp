  ...
  <div class="container-fluid">
    <div class="card card-primary card-outline">
      <div class="card-header">
		<h3><?=$this ->lang ->line('stationery_master')?></h3>
	      	...
	      	<div class="card-body">
		      	<div class="row">
		      		<div class="col-md-4">
		      			<?php  //echo $title; exit; ?>
		      			<?php if(!empty($id)) { ?>
				    		<form class="form-horizontal" role="form" method="post" action="<?php echo base_url(); ?>index.php/Stationery/editPM/<?= $id ?>">
				    			<input type="hidden" name="pm_id" value="<?= $id?>">
				    			<?php } else { ?>
							<form class="form-horizontal" role="form" method="post" action="<?php echo base_url(); ?>index.php/Stationery/add_newPM">
				    			<?php } ?>
				        <div class="form-group">
				       <?=$this ->lang ->line('new_material_code')?> :  <label class="control-label"> <?= $stationary_code_view ?></label>
				       <input type="hidden" name="code" value="<?= $stationary_code_view ?>" > 
					                  ...
					                  <?php 
					                    foreach ($categories as $value) : ?>
					                        <?php 
						                        ...
						                        <label class="control-label"> <?=$this ->lang ->line('stationery')?><?=$this ->lang ->line('name')?> </label>
							...
							<div class="row col-md-12">
								 <label class="control-label"> <?=$this ->lang ->line('company_name')?></label>
										...
										class="form-control" value="<?= $company_name?>" required autofocus>
								</div>
								
					        <span class="help-block"></span>
					            <!--<div class="row col-md-12">
					            	<label class="control-label">Size</label>
					                <input type="text"  placeholder="Enter size" name="bag_size" class="form-control" 
									value="<?= $bag_size?>"  autofocus>
					            </div>-->
							<div class="row col-md-12">
					            	<label class="control-label"> <?=$this ->lang ->line('minimum_inventory_qty')?></label>
								<input type="text"  placeholder="<?=$this ->lang ->line('enter_minimum_inventory_qty')?>" name="minimum_inventory_qty" 
				...
				<div class="col-md-8">
					...
					<table id="example1" class="table table-bordered table-striped">
						...
						<tbody>
							<?php $i=1;foreach($stationeries as $stationery) { ?>
							<tr>
								<td><?= $i ?></td>
								<td><?= $stationery['name'] ?></td>
								<td><?= $stationery['company_name'] ?></td>
								<td><?= $stationery['description'] ?></td>
								<td> <a class="btn btn-xs btn-info btnEdit" href="<?php echo base_url(); ?>index.php/Stationery/index/<?php echo $stationery['id'];?>"><i class="fa fa-edit"></i></a></td>
							</tr>
						<?php $i++;} ?>
						</tbody>
					</table>
