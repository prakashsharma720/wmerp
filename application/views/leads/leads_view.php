	...
	<div class="card card-primary card-outline">
		<div class="card-header">
			...
			<div class="pull-right" style="margin-left:20px;">
				<a class="btn btn-xs btn-danger " href="<?php echo base_url(); ?>uploads/Lead_csv_MO_ERP.xlsx">
		...
		<div class="card-body">
			...
			<form action="<?php echo base_url(); ?>index.php/Leads/importdata" enctype="multipart/form-data"
				method="post" role="form">
				<div class="row">
					<div class="col-md-12">
						<div class="col-md-6">
							<label class="control-label"> Upload File</label><span class="required"> (Only Excel/CSV
								File Import. in given format)</span>
							<input type="file" name="uploadFile" value="" required="required" />
						</div>
						<div class="col-md-6">
							<button type="submit" class="btn btn-success" name="submit" value="submit">Upload Excel/CSV
								File Here</button>
						</div>
			...
			<form method="get">
				<div class="row" id="filter_hide">
							...
							if ($categories): ?>
								...
								foreach ($categories as $value): ?>
									...
									if ($value['category_name'] == @$filtered_value['category_name']): ?>
										<option value="<?= $value['category_name'] ?>" selected><?= $value['category_name'] ?>
										</option>
									<?php else: ?>
					...
					<div class="col-md-4 col-sm-4">
						...
						<input type="text"
							value="<?php if (!empty($filtered_value['from_date'])) {
								echo date('d-m-Y', strtotime($filtered_value['from_date']));
							} ?>"
							data-date-formate="dd-mm-yyyy" name="from_date" class="form-control date-picker"
					...
					<div class="col-md-4 col-sm-4">
						...
						<input type="text"
							value="<?php if (!empty($filtered_value['from_date'])) {
								echo date('d-m-Y', strtotime($filtered_value['upto_date']));
							} ?>"
							data-date-formate="dd-mm-yyyy" name="upto_date" class="form-control date-picker"
							placeholder="dd-mm-yyyy" autocomplete="off" autocomplete="off">
					</div>
				...
				<?php if ($role_id == '1' || $role_id == '2') { ?>
					<div class="col-md-6" style="margin-left:-1%; ">
						<div class="col-md-4 col-sm-3">
							...
							<select id="selectestimateid" style="margin-left:-1px;  width: 206% !important;"
								name="approveaction" class="form-control">
						...
						<div class="col-md-3 col-sm-3" style="margin-top:-50px;margin-left:60%;">
							<button type="submit" class="btn btn-lg transparent_button bulk_action"
								style="font-size: 0.8125rem;"><b>Apply</b>
							</button>
						</div>
					</div>
					...
					<table id="example1" class="table table-bordered table-striped">
						...
						if (!empty($leads)) {
							...
							foreach ($leads as $obj) {
								?>
									<tr>
										<td><input type="checkbox" class="sub_chk" name="sub_chk[]"
										...
										<td>
											...
											if ($obj['is_duplicate'] == 0) { ?>
												-
											<?php } else if ($obj['is_duplicate'] == 1) {
												// print_r($leads_id);
									
												?>
													<p style='color:red' ;>Seems Like Duplicate as</p>
													<a
														href="<?php echo base_url(); ?>index.php/Leads/view/<?php echo $obj['duplicate_lead_code']; ?>"><?php echo $obj['duplicate_lead_code'] ?></a>
											<?php } ?>
										...
										<td>
											...
											<?php if ($role_id !== '3') { ?>
												<a class="btn btn-xs btn-primary btnEdit"
													href="<?php echo base_url(); ?>index.php/Leads/add/<?php echo $obj['id']; ?>"><i
										...
										<div class="modal fade" id="view<?php echo $obj['id']; ?>" role="dialog">
											<div class="modal-dialog">
												...
												<form class="form-horizontal" role="form" method="post"
													action="<?php echo base_url(); ?>index.php/Leads/reminder/<?php echo $obj['id']; ?>">
