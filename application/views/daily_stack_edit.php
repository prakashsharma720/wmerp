...
<style type="text/css">
  ...
  <div class="container-fluid">
    <div class="card card-primary card-outline">
      ...
      <div class="card-body">
			<form class="form-horizontal" role="form" method="post" action="<?php echo base_url(); ?>index.php/Daily_stacking_records/edit_dsr">
			        			...
			        			<tbody id="mainbody">
			        				...
			        				if(!empty($dsr_details)){
			        					$i=1; foreach ($dsr_details as $key => $pr_detail) { 
			        						//print_r ($pr_detail);exit;
			        						$worker_selected=explode(',',$pr_detail['worker_ids']);
			        						//print_r ($worker_selected);exit;
			        						?>
			        				<tr class="main_tr1">
										<td ><?= $i ?></td>
										<td>
										...
										<td style="width: 30%;">
							                  <select class="form-control worders select2 " multiple="multiple" data-placeholder="Select Workers" style="width: 350px !important;" name="worker_ids[]" >
									            ...
									            <?php if ($workers): ?> 
									               ...
									               <!--  <?php 
								               	 ...
								               	 <?php if ($value['name'] == $worker_selected[$key]): ?>
										...
										<td>
											<select name="bag_weight_stack_up[]" class="form-control bag_weight_stack_up" required style="width: 200px;">
												...
												?>
												<?php if ($k== $pr_detail['bag_weight_stack_up']): ?>
													<option value="<?php echo $k;?>" selected><?php echo $v;?>
													</option>
													<?php else :?>
										...
										<td>
											...
											<select name="bag_weight_stack_down[]" class="form-control bag_weight_stack_down" required style="width: 200px;">
												...
												<?php if($packing_sizes):
														foreach($packing_sizes as $k=>$v):
												?>
												<?php if ($k== $pr_detail['bag_weight_stack_down']): ?>
													<option value="<?php echo $k;?>" selected><?php echo $v;?>
													</option>
													<?php else :?>
													<option value="<?php echo $k;?>"><?php echo $v;?>
...
<table id="sample_table1" style="display: none;">
	<tbody>
		<tr class="main_tr1">
			...
			<td style="width: 30%;">
                  <select class="form-control  worders" multiple="multiple" data-placeholder="Select Workers" style="width: 350px !important;" name="worker_ids[]" >
                    <!-- <option value=""> Select Worker</option> -->
		            <?php if ($workers): ?> 
			...
			<td>
   			...
   			<td>
				<input type="text"  placeholder="Total" name="total[]" class="form-control total"  autofocus style="width: 150px;" readonly >
			</td>
