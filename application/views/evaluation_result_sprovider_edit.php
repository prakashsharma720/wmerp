      ...
      <div class="card-body">
		        	...
		        	<div class="row col-md-12">
		        		<div class="col-md-4 col-sm-4 ">
			            	<label class="control-label"><?= $this->lang->line('date') ?> <span class="required">*</span></label>
			                 ...
			                 value="<?php if($date) { echo date('d-m-Y',strtotime($date)); } echo date('d-m-Y')?>" autofocus required >
			            </div>
			             <div class="col-md-4 col-sm-4 ">
						...
						<div class="col-md-4 col-sm-4 ">
			            	 ...
			            	 <select name="service_provider_id" class="form-control select2 service_providers" required="required">
						        <?php
						         if ($sproviders): ?> 
						          <?php 
						            foreach ($sproviders as $value) : ?>
						                <?php 
		        	...
		        	<div class="row col-md-12">
		        		<div class="table-responsive">
			        		<table class="table table-bordered " id="maintable" >
			        			...
			        			<tbody id="mainbody">
			        			<?php
			        					$i=1;foreach ($er_details as $key => $criteria) {
			        					?>
			        					<tr class="main_tr1">
											<td style="width:5%"><?= $i ?></td>
											<td style="width:50%"> 
												<input type="hidden" name="evaluation_criteria_id[]" value="<?= $criteria['evaluation_criteria_id'] ?>">
											...
											<td style="width:10%" class="select_grade">
												<?php  $app_cat = array(
								            		...
								            		echo form_dropdown('marks_obtained[]', $app_cat,$criteria['marks_obtained'],'', 'required="required"')
								            	?>
											</td>
											<!-- <td style="width:10%">
												<input type="text"  placeholder="Enter marks" name="marks_obtained[]" class="form-control marks" 
												oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');"  autofocus required value="<?= $criteria['marks_obtained'] ?>">
											</td> -->
			        			...
			        			<tfoot>
			        				<tr>
			        					...
			        					<td colspan="2">
			        						...
			        						<div class="input-group mb-3">
							                  <input type="text"  placeholder="<?= $this->lang->line('total_marks') ?>" name="total_marks_obtained" class="form-control total_marks"  value="<?= $total_marks_obtained?>" readonly >
			        				...
			        				<tr>
			        					...
			        					<td colspan="2">
			        						<input type="text"  placeholder="<?= $this->lang->line('approval_grade') ?>" name="approval_grade"  value="<?= $approval_grade; ?>" id="approval_grade" readonly >
			        					</td>
			        				</tr>
			        			</tfoot>
			        		</table>
	        	...
	        	<div class="row col-md-12">
		            <div class="col-md-12 col-sm12 ">
	            		...
	            		<textarea type="text" placeholder=" <?= $this->lang->line('enter_remarks') ?>" name="comments" class="form-control" value="<?= $comments?>"><?= $comments?></textarea>
	        		</div>
		       </div>
		        <div class="form-group">
		        	<div class="row col-md-12">
		...
		$(document).on('change','.category',function(){
				...
				$.ajax({
	                ...
	                success: function (response) {
	                	//alert(response);
	                    $(".service_providers").html(response);
	                    $('.select2').select2();
	                }
            	});
			}); 
			var service_provider_id = $('.service_providers').find('option:selected').val();
				//var aa= base_url+"index.php/Meenus/rolewisedata/"+role_id;
				//alert(supplier_id);
				$('.ajax_data').show();
				$.ajax({
	                type: "POST",
	                url:"<?php echo base_url('index.php/Service_providers/getSProviderById/') ?>"+service_provider_id,
	                //data: {id:role_id},
	                dataType: 'html',
	                success: function (response) {
	                	//alert(response);
			...
			$(document).on('change','.service_providers',function(){
				var service_provider_id = $('.service_providers').find('option:selected').val();
				//var aa= base_url+"index.php/Meenus/rolewisedata/"+role_id;
				//alert(supplier_id);
				$('.ajax_data').show();
