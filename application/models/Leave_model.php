...
Class Leave_model extends MY_Model {
  ...
  public function __construct() {
   ...
   //  }
	...
	function getDesignation($author_id) {
    ...
    public function getPreview($id) {
        ...
        if ($data):
            ...
            // $this->data['text'] = $data['text'];
           	$this->data['designation'] = $this->getDesignation($data['designation_id']);
            ...
            $this->data['gate_date'] ='';
            $this->data['gate_time_from'] ='';
            $this->data['gate_time_to'] ='';
            ...
            else if ($data['leave_category'] == 'half'){
                $this->data['halfday_date'] = date('d-m-Y', strtotime($data['halfday_date']));
                $this->data['halfday_type'] = $data['halfday_type'];
            }else if ($data['leave_category'] == 'gatepass'){
                $this->data['gate_date'] = date('d-m-Y', strtotime($data['gate_date']));
                $this->data['gate_time_from'] = date('h:i A',strtotime($data['gate_time_from']));
                $this->data['gate_time_to'] = date('h:i A',strtotime($data['gate_time_to']));
			}
		...
		if ($data){
			...
			if ($user_leave_authorities):
				if ($user_leave_authorities['designation_id'] == 8) {
					...
					//$cc = ['hussain@muskowl.com', 'arushi@muskowl.com'];
			...
			$email = new Email_lib(); // Create an instance of Email_lib
			$email->setNewline = "\r\n"; // Set newline
			$email->fromEmail = 'erp@muskowl.com';
			$email->fromName = 'Musk Owl LLP';
			...
			if ($email->send()) {
				...
				echo "Email failed! Debugging info:<br>";
				echo "<pre>";
				print_r($this->email->print_debugger()); // Print debug info
				exit;
			}
		}
		return FALSE;
	...
	public function SendReplyEmail($leave_id) {
		...
		$empdata      = $this->getDetailsByID($author_id);
		$action_name  = $empdata['name'];
		
		 
		$action_designation = $this->getDesignation($empdata['designation_id']);
		 $config = Array(
		  'protocol' => 'smtp',
		...
		<html>
			...
			<body>
		        ...
		        <div><font size="4" face="trebuchet ms, sans-serif"><?= '.$action_name.'</font></div>
		        <div><b><font size="4" face="trebuchet ms, sans-serif">'.$action_designation.'</font></b></div>
		...
		$this->email->from($author_email); // change it to yours
	...
	public function get_all_holiday_dates()
    {
        $this->db->select('date');
		$this->db->where('flag','0');
        $query = $this->db->get('holidays'); // your holiday table
	...
	public function LeaveApprovalList($conditions = null) 
	{
		//echo "<pre>"; print_r($conditions);
		$this->db->select('leaves.*,employees.name as employee,employees.department_id as department_id,leave_types.leave_type as leave_type');
		$this->db->from('leaves');
		$this->db->join('employees','leaves.employee_id=employees.id','left');
		$this->db->join('leave_types','leaves.leave_type_id=leave_types.id','left');
		if(!empty($conditions)){
			...
			if(!empty($conditions['login_id'])){
				$this->db->where('leaves.employee_id !=',$conditions['login_id'],'both');
				$this->db->where('leaves.leave_status','Pending');
			}
		}else{
			$this->db->where('leaves.leave_status','Pending');
		}
	...
	public function getById($id) {
       $this->db->select('leaves.*,employees.name as username,employees.email as email,employees.author_email as author_email,employees.author_id as author_id,employees.designation_id as designation_id,leave_types.leave_type as leave_type');
		$this->db->from('leaves');
		$this->db->join('employees','leaves.employee_id=employees.id','left');
		$this->db->join('leave_types','leaves.leave_type_id=leave_types.id','left');
...
public function getByIdLavetype($id) {
	...
	public function deleteleave($id){
		// print_r($id);exit;
		if($this->db->delete('leave_taken', "leave_id = ".$id)) return true;
	}
